...
Формат **EXTM3U** появился как расширение формата m3u, который широко применялся для формирования плейлистов со 
списком аудиозаписей. От этого и название – extended m3u. Изначально этот формат плейлистов появился в 
медиапроигрывателе Winamp, но со временем обрел высокую популярность, и его со временем начали поддерживать 
почти все проигрыватели. Формальной спецификации для формата EXTM3U нет, это стандарт де-факто, со всеми 
вытекающими из этого проблемами.
...
Директива #EXTM3U – это обязательный заголовок файла, сообщает устройству, что это плейлист в формате EXTM3U. 
Должна быть первой строкой в плейлисте. 

Также рядом с ней могут указываться дополнительные аттрибуты, такие как ссылка на файл с TV-программой (в случае если
плейлист используется для просмотра IPTV). Аттрибуты не являются обязательными.
...
пробелов между именем атрибута и знаком '=' и т.д..  
...
Далее, следующей строкой идет ссылка на медиаисточник, например поток IPTV или путь к имени файла, если это музыкальный трек. 
...
Разные медиаустройства могут использовать разный набор параметров и трактовать их по своему.